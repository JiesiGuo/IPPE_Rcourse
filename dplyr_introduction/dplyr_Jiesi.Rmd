...
bind_cols(mydata, data.frame(z = 3:1)) # when combinding column-wise,the number of rows must match, but row names are ignored
bind_rows(mydata, data.frame(x = 10, y = "z")) # when combinding row-wise, both the number and names of columns much match
```
