...
* [Window function](http://cran.r-project.org/web/packages/dplyr/vignettes/window-functions.html) for calculating ranking, offsets, and more (not covered below; see [vignette for window-functions](https://github.com/hadley/dplyr/blob/master/vignettes/window-functions.Rmd))
...
```
```(r results='hide')
# nesing method to select State and PV1MATH columns and filter for PV1MATH over 400
filter(select(pisa, State, PV1MATH), PV1MATH > 400)
```
```{r}
# chaining method
pisa %>%
...
```{r }
# for each SCHOOLID, create the mean of students' math and science achievement for each student
